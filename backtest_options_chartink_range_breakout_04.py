"""
User request 14 Sep 2026: backtest the OPTIONS strategy against a bigger
real Chartink alert file ("04 Range Breakout.csv", 07 Aug - 11 Sep 2026
across 26 trading days, 153 unique symbols, CE-only scan) - explicitly
requested as a larger sample to properly evaluate the Nifty gap-down cap
change (120->30 min, deployed 14 Sep 2026) across more real gap-down/
sharp-fall mornings than the smaller "03 Range Breakout.csv" pass had.
Reuses the exact methodology built for backtest_options_chartink_range_
breakout_03.py (itself built on backtest_options_chartink_krishvi_03.py /
backtest_luxury_chartink_longterm_06_range_breakout.py) - Options and
Luxury run an IDENTICAL entry/exit rule set as of 10 Sep 2026 (the
choppy-stocks pre-filter was removed from Options that day specifically
so it would match Luxury's own guard rails). This script differs from
the "03" pass only in which CSV it reads.

Modeled (same as the Krishvi "03" pass):
  - Batch ranking: every same-minute alert batch is ranked by day-change%
    ...
    specifically the BOTTOM N (SELECT_BOTTOM_N_STOCKS=true, prefer_highest
    passed True for CE) - the WEAKEST performers of the batch.
  - MAX_LIVE_POSITIONS_CE capacity (whole batch ignored outright if already
    at capacity, not entered partially).
  - MAX_DAILY_ENTRIES_PER_SYMBOL cap.
  - Same-day RSI-gated loss re-entry block (config.ENABLE_RSI_LOSS_REENTRY_
  ...
  - ENABLE_TRADING_TIME_LIMIT/ALLOWED_TRADING_TIME entry cutoff.
  - Full exit ladder: MAX_LOSS_HIT -> TARGET_HIT -> PROFIT_PROTECTION_HIT
    ...
    (dynamic SL) -> SUPERTREND_EXIT. All read live from Options/config.py
    at import time.
...
helper.py - a shared helper that reconstructs evaluate_nifty_open_
condition/is_nifty_recovering/should_delay_ce_entry from NIFTY50's own
historical 1-min candles, faithfully including the live code's one-shot-
per-day caching quirk (whichever alert is first to ask that day "locks
in" the day's gap/fall judgment). See that module's own docstring.

NOT modeled (disclosed, not silently dropped - same standing limitations
as every prior pass this session):
  - SL-L broker-side stop-loss (real effect can only tighten a loss, never
    worsen it).
  - cross_strategy_registry (Options/Futures/Luxury mutual symbol lock) -
    same standing limitation as every backtest script this session.

Run via SSH on the droplet (never locally while the live bot's own
session is active):
    uv run python backtest_options_chartink_range_breakout_04.py [PP_BEFORE] [PP_AFTER] [GIVEBACK_PCT]
...
CSV_PATH = Path("/root/apps/traderBoy/04 Range Breakout.csv")
CACHE_DIR = Path("/private/tmp/claude-501/-Users-kushalgaur-Desktop-projects-trading-traderBoy/"
                  "60a0e686-2110-4e20-bad5-fe817a57a72a/scratchpad/options_chartink_backtest_cache_rb04")
...
MAX_LIVE_POSITIONS_CE = options_config.MAX_LIVE_POSITIONS_CE
...
MAX_LOSS_BEFORE = options_config.MAX_LOSS_PER_TRADE_RS_BEFORE_CUTOFF
MAX_LOSS_AFTER = options_config.MAX_LOSS_PER_TRADE_RS_AFTER_CUTOFF
...
DYNAMIC_SL_STEP_PCT_CE = options_config.DYNAMIC_SL_STEP_PCT_CE
...
ENABLE_GAP_DOWN_CE_DELAY = options_config.ENABLE_GAP_DOWN_CE_DELAY
...
def fetch_option_1min_cached(security_id: str) -> dict:
    ...
    liquidity-guard exit - see exit_ladder_backtest_helper.py). A cache
    file written before that still validates as JSON but lacks "volumes" -
    treated as a cache miss so it gets re-fetched with the field present,
    rather than silently running the liquidity guard on missing data."""
...
def main():
    ...
    (CACHE_DIR / "results_range_breakout_04.json").write_text(json.dumps(trades, default=str, indent=2))
