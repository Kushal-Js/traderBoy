...
| `POST /chartink/webhook-swing-enter` | Mode-aware (added 1 Sep 2026, extended to 3-way 1 Sep 2026): directly enters a Swing basket (futures + ATM PE hedge, basket OR basket_hedge mode - the mechanics are identical, only the STORE and later exit behavior differ) OR just the futures leg (sequential mode - the PE only ever comes in later, as the hedge once that leg exits) for the stock(s) in the payload - a manual/explicit trigger, not a scan-ranked selection. Returns `status=ignored` while `SWING_STRATEGY_ENABLED=false` (the current deployed default) |
