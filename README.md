...
| `Swing/swing_main.py` | "Swing" strategy's FastAPI router + lifespan. TWO trading-mechanics modes coexist, switched by `config.STRATEGY_MODE` (added 1 Sep 2026): "basket" (buys 1 lot of a stock's FUTURES contract hedged with 1 lot of its ATM PE option, together, all-or-nothing) and "sequential" (**now the default/active mode**) - futures only on entry; on exit, SELL futures and BUY the ATM PE as a hedge instead; exit the PE either on its own ₹2k loss cap (back to watching) or once the entry condition fires again (which also immediately re-buys futures - "keep this loop going"). Mode-aware reconciliation, webhook dispatch, and observability endpoints throughout. **Deployed DISABLED** (`SWING_STRATEGY_ENABLED=false`) regardless of mode |
| `Swing/trading_engine.py` | Basket mode (unchanged): the all-or-nothing entry (compensating rollback if either leg fails), basket exit, broker-position reconciliation (pairs a Swing-attributed futures leg + PE leg back into a basket at startup). Sequential mode (added 1 Sep 2026): `_enter_futures_for_stock`/`_swap_futures_to_pe`/`_swap_pe_to_futures`/`_exit_pe_to_watching` implement the 4-state loop (NONE/FUTURES/PE, each swap "fail safe to flat" on the next leg's own failure), `reconcile_sequential_positions` recovers a LONE broker leg (the NORMAL shape here, unlike basket mode). BOTH modes share the identical entry/exit SIGNAL (`_is_price_confirmed_above_prev_close` - true at market open if open >= prev close, OR later intraday the first time price reaches/crosses above prev close, LATCHES once confirmed - AND the dual-timeframe Supertrend crossover for entry, a 5-min crossed-below for exit). `monitor_loop` dispatches by `config.STRATEGY_MODE` each tick. Entry dedup deliberately does NOT go through `cross_strategy_registry.py` (removed 1 Sep 2026, user decision - Swing is an "independent strategy") - each mode's own store provides its own atomic claim instead. Every real state transition (basket enter/exit, sequential entry/swap/exit, including a mid-swap failure left flat) also fires `_record_swing_event()` - a durable, queryable `history/swing_events` log capturing WHY, added 1 Sep 2026 right after Swing went live ("log everything under history folder for 'Swing' package also") |
| `Swing/position_store.py` | `Basket`/`Leg` dataclasses + `BasketStore` (basket mode - 2 linked legs per basket) and `SequentialPositionStore` (added 1 Sep 2026, sequential mode - at most ONE `Leg` per symbol at a time, whose own `option_type` says which instrument is currently held) |
...
| `Swing/config.py` | All tunables for Swing, `SWING_`-prefixed env vars, including the `STRATEGY_ENABLED` on/off flag (real orders, stays `false`), `STRATEGY_MODE` ("basket" \| "sequential", added 1 Sep 2026, defaults to "sequential"), `PE_MAX_LOSS_RS` (sequential mode's PE hedge loss cap, default 2000), and the independent `PAPER_TRADING_ENABLED` flag |
...
| `Swing/paper_engine.py` | Paper-trading engine (added 1 Sep 2026) - mirrors whichever mode (`config.STRATEGY_MODE`) is active, reusing the exact same, unchanged entry/exit signal as real trading but simulating fills at current LTP instead of placing real orders. Basket mode: `PaperBasketStore`, own log (`swing_paper_trades`, one row per completed basket). Sequential mode (added 1 Sep 2026): `SequentialPaperStore`, its OWN log (`swing_sequential_paper_trades`, one row per completed LEG, never mixed with basket mode's rows) - `_enter_paper_futures`/`_swap_paper_futures_to_pe`/`_swap_paper_pe_to_futures`/`_exit_paper_pe_to_watching` mirror the real sequential state machine exactly. Both modes log each leg's real margin requirement (`get_margin_required`) and the account's fund snapshot (`get_fund_limits`) at entry for offline analysis - best-effort, never blocks/invalidates a paper trade if that fetch fails |
...
| `POST /chartink/webhook-swing-enter` | Mode-aware (added 1 Sep 2026): directly enters a Swing basket (futures + ATM PE hedge, basket mode) OR just the futures leg (sequential mode - the PE only ever comes in later, as the hedge once that leg exits) for the stock(s) in the payload - a manual/explicit trigger, not a scan-ranked selection. Returns `status=ignored` while `SWING_STRATEGY_ENABLED=false` (the current deployed default) |
| `POST /chartink/webhook-swing-watchlist` | Adds stock(s) to Swing's watchlist for continuous monitoring (shared by both modes) |
| `GET /swing/positions` | Basket mode's own live + closed baskets (both legs of each) - reads empty under sequential mode |
| `GET /swing/sequential-positions` | Sequential mode's own live + closed legs (added 1 Sep 2026) - reads empty under basket mode |
| `GET /swing/watchlist` | Swing's current watchlist |
| `POST /swing/square-off-now` | Swing's own manual kill-switch, mode-aware - closes both legs of every live basket (basket mode) or the single held leg of every symbol, returning it to plain watching (sequential mode) |
| `GET /swing/paper-trades` | Basket mode's own PAPER trades - live + completed simulated baskets, per-leg and total P&L, win rate, plus each leg's real margin requirement + the account's fund snapshot at entry for offline analysis. Always reachable regardless of `SWING_PAPER_TRADING_ENABLED`. Reads empty under sequential mode |
| `GET /swing/sequential-paper-trades` | Sequential mode's own PAPER trades (added 1 Sep 2026) - one row per completed leg (not per basket), same margin/funds logging. Reads empty under basket mode |
