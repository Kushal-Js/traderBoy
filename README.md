...
| `Futures/trading_engine.py` | Near-verbatim copy of `Options/trading_engine.py`'s ranking/entry/exit logic against this package's own config/position_store, including concurrent entry placement, broker-position reconciliation (added 31 Aug 2026 - filtered so it can never import a position that's actually Options'), and its own independent daily re-entry cap (`config.MAX_DAILY_ENTRIES_PER_SYMBOL`, added 1 Sep 2026) |
...
| `Luxury/trading_engine.py` | Near-verbatim copy of `Options/trading_engine.py`'s ranking/entry/exit logic against this package's own config/position_store, including concurrent entry placement, broker-position reconciliation (filtered so it can never import a position that's actually Options'/Futures' own), and its own independent daily re-entry cap (`config.MAX_DAILY_ENTRIES_PER_SYMBOL`, added 1 Sep 2026). Entry/exit condition VALUES matched to Options' own actual deployed config 1 Sep 2026 (`TARGET_PCT`/`STOP_LOSS_PCT`/`DYNAMIC_SL_STEP_PCT_PE`/`ENABLE_SQUARE_OFF` had all drifted since Luxury was first built) |
