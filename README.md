...
| `Swing/config.py` | All tunables for Swing, `SWING_`-prefixed env vars, including the `STRATEGY_ENABLED` on/off flag (real orders, stays `false`), `STRATEGY_MODE` ("basket" \| "sequential" \| "basket_hedge", added 1 Sep 2026, defaults to "basket_hedge"), `PE_MAX_LOSS_RS` (the PE hedge's own loss cap, shared by sequential AND basket_hedge modes, default 2000), `PE_PROFIT_LOCK_RS` (basket_hedge mode only, added 1 Sep 2026, its own profit-lock exit, default 2000), `WATCHLIST_DAILY_PRUNE_ENABLED`/`DAILY_EMA_PERIOD`/`DAILY_TREND_LOOKBACK_DAYS` (added 1 Sep 2026, the daily TREND-based watchlist prune's own tunables), `CHARTINK_WATCHLIST_SCAN_ENABLED`/`CHARTINK_WATCHLIST_SCAN_URL`/`CHARTINK_WATCHLIST_SCAN_CLAUSE`/`CHARTINK_WATCHLIST_SCAN_TIME` (added 1 Sep 2026, the daily Chartink scan pull's own tunables), `WATCHLIST_STALE_AGE_PRUNE_ENABLED`/`WATCHLIST_STALE_AGE_DAYS` (added 1 Sep 2026, the second, AGE-based watchlist prune's own tunables, default 10 days), and the independent `PAPER_TRADING_ENABLED` flag |
