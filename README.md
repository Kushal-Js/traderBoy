...
| `Swing/swing_main.py` | "Swing" strategy's FastAPI router + lifespan - buys 1 lot of a stock's FUTURES contract hedged with 1 lot of its ATM PE option, as an all-or-nothing "basket" (user request 31 Aug 2026). Entry/exit condition logic is deliberately deferred to the user - see `Swing/trading_engine.py`'s own docstring for the two extension points. **Deployed DISABLED** (`SWING_STRATEGY_ENABLED=false`) |
| `Swing/trading_engine.py` | The all-or-nothing basket entry (compensating rollback if either leg fails - "do both of these or neither"), basket exit, and broker-position reconciliation (pairs a Swing-attributed futures leg + PE leg back into a basket at startup - important here since Swing baskets carry across restarts by design, no EOD square-off) |
