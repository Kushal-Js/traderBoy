...
| `Swing/watchlist.py` | In-memory watchlist Swing's monitor loop polls, populated via `POST /chartink/webhook-swing-watchlist`, (added 31 Aug 2026) by hand-editing `data/watchlist` on the server (one symbol per line, `#` comments allowed - re-synced every monitor tick, same hot-reload UX as `choppy_stocks.py`), and (added 1 Sep 2026) automatically once daily pre-market by the Chartink scan pull above. Symbols also get automatically REMOVED once daily by `trading_engine._daily_watchlist_prune_tick` (see its own row above) |
| `Swing/config.py` | All tunables for Swing, `SWING_`-prefixed env vars, including the `STRATEGY_ENABLED` on/off flag (real orders, stays `false`), `STRATEGY_MODE` ("basket" \| "sequential" \| "basket_hedge", added 1 Sep 2026, defaults to "basket_hedge"), `PE_MAX_LOSS_RS` (the PE hedge's own loss cap, shared by sequential AND basket_hedge modes, default 2000), `PE_PROFIT_LOCK_RS` (basket_hedge mode only, added 1 Sep 2026, its own profit-lock exit, default 2000), `WATCHLIST_DAILY_PRUNE_ENABLED`/`DAILY_EMA_PERIOD`/`DAILY_TREND_LOOKBACK_DAYS` (added 1 Sep 2026, the daily watchlist prune's own tunables), `CHARTINK_WATCHLIST_SCAN_ENABLED`/`CHARTINK_WATCHLIST_SCAN_URL`/`CHARTINK_WATCHLIST_SCAN_CLAUSE`/`CHARTINK_WATCHLIST_SCAN_TIME` (added 1 Sep 2026, the daily Chartink scan pull's own tunables), and the independent `PAPER_TRADING_ENABLED` flag |
