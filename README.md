...
| `Swing/trading_engine.py` | The all-or-nothing basket entry (compensating rollback if either leg fails - "do both of these or neither"), basket exit, broker-position reconciliation (pairs a Swing-attributed futures leg + PE leg back into a basket at startup - important here since Swing baskets carry across restarts by design, no EOD square-off), and the entry/exit signal (added 31 Aug 2026, extended same day): entry when today's open is greater than yesterday's close AND the 5-min close crosses above its Supertrend AND the 1-min close is above/crosses above its own; exit when the 5-min close crosses below its Supertrend. The gap-up check is cached once per symbol per trading day (cheap, short-circuits the pricier Supertrend checks); the Supertrend half is a self-contained dual-timeframe crossover detector, independent of Options/dhan_client.py's own single-timeframe Supertrend cache to avoid any risk to that already-live exit protection for Options/Futures/Luxury. Entry dedup deliberately does NOT go through `cross_strategy_registry.py` (removed 1 Sep 2026, user decision - Swing is an "independent strategy") - `basket_store.reserve_symbol()`'s own atomic claim is Swing's own separate protection, so Swing and Options/Futures/Luxury can now independently enter the same underlying stock at the same instant |
