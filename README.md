...
| `choppy_stocks.py` | Weekly "choppy stocks" exclusion list for the Options strategy - underlyings whose single F&O lot exceeds `LOT_SIZE_THRESHOLD` (6000 units), refreshed automatically every Monday 12:00 PM IST, persisted to `choppy/choppy_stocks.json` (gitignored runtime data, same convention as `history/`). Read via `GET /choppy-stocks`; enforced in `Options/option_main.py` (excluded before ranking) and `Options/trading_engine.py` (belt-and-suspenders check at entry) |
...
| `GET /choppy-stocks` | Stocks the Options strategy currently won't enter new positions in (lot size > 6000) - `generated_at`/`threshold`/`stocks` from the weekly-refreshed `choppy/choppy_stocks.json`. See `choppy_stocks.py` |
