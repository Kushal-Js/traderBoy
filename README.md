...
| `Swing/trading_engine.py` | The all-or-nothing basket entry (compensating rollback if either leg fails - "do both of these or neither"), basket exit, broker-position reconciliation (pairs a Swing-attributed futures leg + PE leg back into a basket at startup - important here since Swing baskets carry across restarts by design, no EOD square-off), and the entry/exit Supertrend signal (added 31 Aug 2026, user request): entry when the 5-min close crosses above its Supertrend AND the 1-min close is above/crosses above its own; exit when the 5-min close crosses below its Supertrend. A self-contained dual-timeframe crossover detector, independent of Options/dhan_client.py's own single-timeframe Supertrend cache to avoid any risk to that already-live exit protection for Options/Futures/Luxury |
