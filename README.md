...
| `Swing/swing_main.py` | "Swing" strategy's FastAPI router + lifespan - buys 1 lot of a stock's FUTURES contract hedged with 1 lot of its ATM PE option, as an all-or-nothing "basket" (user request 31 Aug 2026). **Deployed DISABLED** (`SWING_STRATEGY_ENABLED=false`) |
| `Swing/trading_engine.py` | The all-or-nothing basket entry (compensating rollback if either leg fails - "do both of these or neither"), basket exit, broker-position reconciliation (pairs a Swing-attributed futures leg + PE leg back into a basket at startup - important here since Swing baskets carry across restarts by design, no EOD square-off), and the entry/exit signal (added 31 Aug 2026, entry's price gate RELAXED 1 Sep 2026 from a strict gap-up): entry when today's price is confirmed at/above yesterday's close (`_is_price_confirmed_above_prev_close` - true at market open if open >= prev close, OR later intraday the first time the current price reaches/crosses above prev close; LATCHES permanently once confirmed) AND the 5-min close crosses above its Supertrend AND the 1-min close is above/crosses above its own; exit when the 5-min close crosses below its Supertrend. The price-confirmation check is cached once per symbol per trading day (cheap, short-circuits the pricier Supertrend checks); the Supertrend half is a self-contained dual-timeframe crossover detector, independent of Options/dhan_client.py's own single-timeframe Supertrend cache to avoid any risk to that already-live exit protection for Options/Futures/Luxury. Both legs of a basket always enter/exit together (unchanged by the price-gate update). Entry dedup deliberately does NOT go through `cross_strategy_registry.py` (removed 1 Sep 2026, user decision - Swing is an "independent strategy") - `basket_store.reserve_symbol()`'s own atomic claim is Swing's own separate protection, so Swing and Options/Futures/Luxury can now independently enter the same underlying stock at the same instant |
| `Swing/position_store.py` | `Basket`/`Leg` dataclasses + `BasketStore` - Swing's own independent capacity/dedup, tracking 2 linked legs per basket rather than a single position |
...
| `Swing/config.py` | All tunables for Swing, `SWING_`-prefixed env vars, including the `STRATEGY_ENABLED` on/off flag (real orders, stays `false`) and the independent `PAPER_TRADING_ENABLED` flag (added 1 Sep 2026) |
...
| `Swing/paper_engine.py` | Paper-trading engine (added 1 Sep 2026, user request - "enable paper trading for tomorrow... keep track of trades, history and profit loss also like real trades in files") - reuses the exact same, unchanged entry/exit signal as real trading, but simulates both legs' fills at current LTP instead of placing real orders (never calls `place_market_order`/`wait_for_order_result`). Its own `PaperBasketStore`, own on-disk log (`swing_paper_trades`, completely separate from real trade history), own poll loop (`paper_poll_loop`) gated by `PAPER_TRADING_ENABLED` independently of `STRATEGY_ENABLED`. Also logs each leg's real margin requirement (`get_margin_required`) and the account's fund snapshot (`get_fund_limits`) at entry, purely for offline analysis - best-effort, never blocks/invalidates a paper trade if that fetch fails |
...
| `POST /chartink/webhook-swing-enter` | Directly enters a Swing basket (futures + ATM PE hedge) for the stock(s) in the payload - a manual/explicit trigger, not a scan-ranked selection. Returns `status=ignored` while `SWING_STRATEGY_ENABLED=false` (the current deployed default) |
| `POST /chartink/webhook-swing-watchlist` | Adds stock(s) to Swing's watchlist for continuous monitoring, ahead of the entry/exit logic still to be defined |
| `GET /swing/positions` | Swing's own live + closed baskets (both legs of each) |
| `GET /swing/watchlist` | Swing's current watchlist |
| `POST /swing/square-off-now` | Swing's own manual kill-switch - closes both legs of every live basket |
| `GET /swing/paper-trades` | Swing's PAPER trades (added 1 Sep 2026) - live + completed simulated baskets, per-leg and total P&L, win rate, and (added same day) each leg's real margin requirement + the account's fund snapshot at entry for offline analysis. Always reachable regardless of `SWING_PAPER_TRADING_ENABLED` so past results stay visible after paper trading is turned off |
