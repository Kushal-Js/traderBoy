...
from . import config
from .dhan_client import OrderResult, OrderStatus, dhan_wrapper
...
def current_max_loss_per_trade_rs(option_type: str) -> float:
    """MAX_LOSS_PER_TRADE_RS_BEFORE_CUTOFF_{CE,PE} before config.
    RISK_THRESHOLD_CUTOFF_TIME, the matching _AFTER_CUTOFF_{CE,PE} from
    that instant on for the rest of the day (user request 31 Aug 2026,
    CE/PE split added 14 Sep 2026 - see config.py's own comment for the
    rationale of each). Evaluated fresh on every _exit_reason_for() call
    (not cached), so a position that's been open since before the cutoff
    is still re-evaluated against the tighter afternoon cap once the
    clock crosses it, same as every other time-of-day gate in this file."""
    if option_type == "CE":
...
async def _process_one_entry(symbol: str, option_type: str) -> dict:
    ...
    if config.ENABLE_RSI_LOSS_REENTRY_BLOCK:
        loss_hits_today = await loop.run_in_executor(
            None, loss_exit_count_today, "Options", symbol, ("MAX_LOSS_HIT",), datetime.now(),
        )
    ...
    # just above, this one never expires today once tripped. BROADENED 18
    # Sep 2026 (real incident, user request): now counts ANY exit that
    # closed at a genuine monetary loss (trade_history.loss_count_today),
    # not just MAX_LOSS_HIT/STOP_LOSS_HIT - a real ATHERENERG loss via
    # SUPERTREND_EXIT/EMA_CROSS_EXIT the old reason-scoped count never saw
    # let a 3rd same-day entry through after two straight real losses.
    if config.LOSS_REPEAT_BLOCK_ENABLED or config.LOSS_REENTRY_TREND_CHECK_ENABLED:
        loss_count = await loop.run_in_executor(None, loss_count_today, "Options", symbol, datetime.now())
    ...
    # Volume-floor entry gate (promoted from shadow-mode logging to a
    # real live gate, 16 Sep 2026 - see config.VOLUME_FLOOR_GATE_ENABLED's
    # own docstring for the backtest evidence). Checked last among the
    # guard checks (needs a real REST call, unlike the cheap disk-read
    # checks above) but still before any capacity/cross-strategy claim,
    # so a thin-volume signal never burns either for a trade that's about
...
async def _place_broker_stop_loss_if_enabled(
    ...
    # Real incident, 21 Sep 2026 (LICI, Options): for a cheap-premium,
    # large-quantity position whose entire notional value (fill_price *
    # quantity) is already BELOW the rupee MAX_LOSS cap, this formula
    # computes a NEGATIVE trigger price (confirmed live: fill=0.70,
    # qty=1400, max_loss_cap=3500 -> trigger=-1.80) - Dhan correctly
    # rejects it (no order_id returned), logged as an ERROR even though
    # nothing is actually broken: the rupee cap can never bind for a
    # position whose worst case (the option going to zero) already loses
    # less than the cap, so there is nothing for a rupee-based broker
    # order to protect here in the first place - the percentage-based
    # STOP_LOSS_PCT hard stop (enforced by the regular poll/tick
    # MAX_LOSS_HIT/STOP_LOSS_HIT check below, same as always) is what
    # actually bounds this position's downside. Skip cleanly instead of
    # attempting an order that can only ever fail.
    if trigger_price <= 0:
        logger.info(
...
async def _enter_single_position(symbol: str, option_type: str = config.OPTION_TYPE) -> dict:
    ...
    try:
        ...
        logger.exception(
            "%s: could not confirm BUY order %s's fill status within %.0fs - treating as still "
            "pending so the 'not yet terminal' handling below applies (releases order ownership to "
            "the background sync instead of blocking here)",
...
async def _exit_position(symbol: str, position: Position, exit_price: float, reason: str) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "%s: could not confirm SELL order %s's fill status within %.0fs - treating as still "
                "pending so the 'not yet terminal' handling below applies (pending_exit_order_id "
                "stays set, background sync resolves it)",
                symbol, order_id, _ORDER_RESULT_TIMEOUT_SECONDS,
            )
            result = OrderResult(order_id=order_id, status=OrderStatus.TRANSIT, remark="order_confirmation_timeout",
                                  fill_price=0.0, filled_quantity=0, is_amo=is_amo)
...
# Bounds every blocking Dhan REST call in this file (LTP reads, order-
# status/order-management calls) - see _get_ltp's own docstring for the
# real incident (Swing, 22 Sep 2026) this generalizes the fix from; none
# of these had a bound before 23 Sep 2026, so a single hung dhanhq call
# could stall this package's entire monitor_loop tick (asyncio.gather
# waits for every position's check to finish before the loop can sleep
# and move on) for however long the SDK's own default HTTP timeout takes.
_LTP_FETCH_TIMEOUT_SECONDS = 10.0

# Single-call functions (refresh_order_status/check_if_order_filled/
# cancel_order).
...
async def _get_ltp(trading_symbol: str) -> Optional[float]:
    ...
    Both REST calls (get_option_ltp, get_last_historical_close) are bounded
    by _LTP_FETCH_TIMEOUT_SECONDS - added 23 Sep 2026 after a live-code
    audit found this package never got the timeout fix Swing's own
    identical _get_ltp received for its 22 Sep 2026 monitor-loop freeze
    (dhanhq's SDK has no built-in per-call timeout short enough to prevent
    ...
    async with dhan_wrapper.ltp_rest_fallback_semaphore:
        try:
            ...
            fallback = await asyncio.wait_for(
                loop.run_in_executor(None, dhan_wrapper.get_last_historical_close, trading_symbol),
                timeout=_LTP_FETCH_TIMEOUT_SECONDS,
            )
            if fallback is None:
                raise
...
async def _capture_supertrend_entry_candle(loop, underlying_symbol: str) -> tuple[Optional[datetime], Optional[float]]:
    ...
    Returns (entry_candle_start, entry_underlying_price) - the second value
    is the underlying's own last-closed-candle price at this same instant,
    read off the exact same refresh this function already does (no extra
    Dhan call), stored on Position.entry_underlying_price for the
    minimum-underlying-move confirmation gate (added 18 Sep 2026 - see
    reversal_filters.check_underlying_move_confirms_exit)."""
...
async def _sync_pending_orders() -> None:
    ...
    entries not yet promoted to a live Position, outstanding exit orders
    on live positions, AND (added 15 Sep 2026, real incident) a plain
    market-hours BUY order that's been stuck non-terminal (PENDING/
    TRANSIT) for longer than config.STALE_ENTRY_ORDER_TIMEOUT_SECONDS -
    see that setting's own docstring for the ICICIPRULI incident this
    fixes. Cheap - normally a no-op, since orders placed during market
    hours almost always resolve within wait_for_order_result's own retry
    budget; this only does real work for a genuinely stuck order (AMO
    waiting for the next session, or the rare stuck-for-minutes case)."""
    loop = asyncio.get_running_loop()
    ...
    for order in pending_entries:
        try:
            result = await asyncio.wait_for(
                loop.run_in_executor(None, dhan_wrapper.refresh_order_status, order.order_id, order.is_amo),
                timeout=_ORDER_STATUS_TIMEOUT_SECONDS,
            )
        except Exception:  # noqa: BLE001
        ...
        # Only a plain order placed during live market hours that's stuck
        # this long indicates a real problem (see config.STALE_ENTRY_
        # ORDER_TIMEOUT_SECONDS's own docstring - the ICICIPRULI incident).
        if order.is_amo:
            continue
        ...
        try:
            await asyncio.wait_for(
                loop.run_in_executor(None, dhan_wrapper.cancel_order, order.order_id),
                timeout=_ORDER_STATUS_TIMEOUT_SECONDS,
            )
        except Exception:  # noqa: BLE001
            logger.exception(
        ...
        # Re-verify broker truth after cancelling - it may have filled in
        # the exact instant the cancel was racing against (same
        # reasoning as _exit_position's own post-cancel reconciliation).
        try:
            broker_qty = await loop.run_in_executor(
    ...
    while True:
        try:
            ...
            if config.CLIMACTIC_GUARD_ENABLED:
                await climactic_entry_guard.poll_pending("Options")
