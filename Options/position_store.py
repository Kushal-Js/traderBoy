...
class Position:
    ...
    def current_trailing_sl(self) -> float:
        ...
        - ENABLE_TRAILING_SL: continuous - floor trails TRAILING_SL_PCT
          below highest_price.
        - ENABLE_DYNAMIC_SL: stepped ("ratchet") - every step % (per
          option type - config.DYNAMIC_SL_STEP_PCT_CE / _PE) the price has
          climbed from entry, the floor moves up DYNAMIC_SL_INCREASE_PCT
          of entry price. E.g. at a 7% step / 1% raise, a position up 15%
          from entry has completed 2 steps, raising the floor from
          -STOP_LOSS_PCT to -(STOP_LOSS_PCT - 2%). Enough accumulated
          steps can push the floor above entry price entirely, locking in
          a guaranteed profit - not capped, since that's the intended
          behavior of a ratchet once the price has moved far enough in the
          trade's favor. TARGET_PCT itself is never touched by this.
...
def _cap_for(option_type: str) -> int:
    ...
    # Opening-burst extra CE slot (19 Sep 2026) - see config.BURST_CAPACITY_ENABLED's
    # own comment for the backtest this is based on. CE only; PE was never
    # modeled. A position entered under this extra slot is an ordinary
    # Position - no other code path needs to know it came from here, and
    # once the window passes, effective capacity just reverts to base (an
    # already-open position from the extra slot keeps running - capacity
    # checks only ever gate NEW entries, never evict one).
