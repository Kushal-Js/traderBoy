...
| `test_cross_strategy_registry.py` | `cross_strategy_registry.py`'s claim/release semantics, and REAL concurrent races (via `asyncio.gather`, not simulated) between Options/Futures/Luxury's actual `_process_one_entry` functions for the same stock - exactly one winner every time, the loser rejected before its own broker check, unrelated symbols never contending |
