...
scratch_dir = Path(tempfile.mkdtemp(prefix="dhanboy_options_broker_stop_loss_test_"))
...
def install_all_dhan_mocks(stop_loss_order_id_factory=None, fail_stop_loss_placement=False,
    ...
    # Only _check_one_position/_get_ltp exercises this pair (tests 1-12
    # never did), so this was previously missing here without symptom -
    # mocked defensively now that a new test needs it.
...
async def test_1_real_entry_places_broker_stop_with_correct_trigger_and_limit():
    ...
    real_enabled = ote.config.BROKER_STOP_LOSS_ENABLED
    real_cutoff = ote.config.RISK_THRESHOLD_CUTOFF_TIME
    ote.config.BROKER_STOP_LOSS_ENABLED = True
    ote.config.RISK_THRESHOLD_CUTOFF_TIME = "23:59"  # force "before cutoff" -> MAX_LOSS_PER_TRADE_RS_BEFORE_CUTOFF
    ...
    try:
        ...
        restore()
        ote.config.BROKER_STOP_LOSS_ENABLED = real_enabled
        ote.config.RISK_THRESHOLD_CUTOFF_TIME = real_cutoff
...
async def test_2_stop_loss_placement_failure_does_not_block_entry():
    ...
    try:
        result = await ote._process_one_entry("TCS", "CE")
...
async def test_3_broker_stop_already_filled_closes_position_directly():
    ...
    odc.dhan_wrapper.unsubscribe_option_price = lambda ts: None
    try:
        result = await ote._check_broker_stop_already_filled("INFY", position)
        ...
        matches = [t for t in closed_trades if t["strategy"] == "Options" and t["underlying_symbol"] == "INFY"]
...
async def test_4_broker_stop_still_resting_leaves_position_untouched():
    ...
    try:
        result = await ote._check_broker_stop_already_filled("WIPRO", position)
...
async def test_5_broker_stop_rejected_falls_through_without_crashing():
    ...
    try:
        result = await ote._check_broker_stop_already_filled("SBIN", position)
        assert result is False, result
        ...
        # Real incident 23 Sep 2026 (Luxury MCX): this field used to stay pointed at the now-dead
        # order forever, so every subsequent tick re-logged the same warning AND re-hit Dhan for an
        ...
        result2 = await ote._check_broker_stop_already_filled("SBIN", position)
...
async def test_6_real_target_hit_finds_and_cancels_the_resting_broker_stop():
    ...
    cancelled_order_ids = []
    try:
        entry = await ote._process_one_entry("HDFCBANK", "CE")
        ...
        await ote._exit_position("HDFCBANK", position, 60.0, "TARGET_HIT")
...
async def test_7_disabled_flag_never_places_the_broker_stop():
    ...
    ote.position_store = store
    real_enabled = ote.config.BROKER_STOP_LOSS_ENABLED
    ote.config.BROKER_STOP_LOSS_ENABLED = False
    restore, placed_orders, stop_loss_calls = install_all_dhan_mocks()
    try:
        result = await ote._process_one_entry("AXISBANK", "CE")
        assert result["status"] == "entered", result
...
async def test_8_partial_fill_on_resting_stop_never_oversells():
    ...
    real_enabled = ote.config.BROKER_STOP_LOSS_ENABLED
    ote.config.BROKER_STOP_LOSS_ENABLED = True
    # See test_6's identical comment - must capture "real_*" BEFORE
    ...
    cancelled_order_ids = []
    try:
        entry = await ote._process_one_entry("ITC", "CE")
        assert entry["status"] == "entered", entry
        ...
        await ote._exit_position("ITC", position, 60.0, "TARGET_HIT")
...
async def test_9_stop_fully_filled_during_cancel_race_reconciles_without_a_fresh_sell():
    ...
    try:
        entry = await ote._process_one_entry("MARUTI", "CE")
        assert entry["status"] == "entered", entry
        ...
        await ote._exit_position("MARUTI", position, 60.0, "TARGET_HIT")
...
async def test_10_sl_l_still_cancelled_when_the_order_book_scan_misses_it():
    ...
    cancelled_order_ids = []
    try:
        entry = await ote._process_one_entry("BAJFINANCE", "CE")
        assert entry["status"] == "entered", entry
        ...
        await ote._exit_position("BAJFINANCE", position, 60.0, "PROFIT_PROTECTION_HIT")
...
async def test_11_rejected_exit_rechecks_broker_on_the_first_failure():
    ...
    try:
        ...
        orders_before = len(placed_orders)
        await ote._exit_position("TATAMOTORS", position, 40.25, "MAX_LOSS_HIT")
...
async def test_12_still_pending_non_amo_exit_defers_instead_of_closing():
    """Regression for PAGEIND, 17 Sep 2026: a LIQUIDITY_GUARD_ZERO_VOLUME
    exit's own market SELL couldn't find an immediate counterparty (the
    same illiquidity that triggered the exit also stalled the exit order
    itself), stayed PENDING past wait_for_order_result's own poll budget,
    and the OLD code fell straight through to close_position() - silently
    recording the position as closed while the real SELL sat unfilled at
    the broker. On the next restart, attribute_open_broker_position could
    no longer match the still-open real position to Options (our own
    history said "closed"), orphaning it with no owner. The fix mirrors
    the entry side: when wait_for_order_result returns a non-terminal,
    non-AMO status, defer to _sync_pending_orders (which already re-
    checks ANY pending exit order every tick) instead of assuming the
    fill happened."""
    store = ops.PositionStore()
    ote.position_store = store
    restore, placed_orders, stop_loss_calls = install_all_dhan_mocks()
    ...
    try:
        position = Position(
            underlying_symbol="PAGEIND", option_trading_symbol="PAGEIND FAKE EXP PE",
            option_type="PE", quantity=20, lot_size=20, entry_price=100.0, highest_price=100.0,
            target_price=130.0, hard_stop_loss=84.0, order_id="OID-ENTRY", product_type="MARGIN",
        )
        store.live_positions["PAGEIND"] = position

        assert await store.try_start_exit("PAGEIND")
        await ote._exit_position("PAGEIND", position, 90.0, "LIQUIDITY_GUARD_ZERO_VOLUME")

        assert "PAGEIND" in store.live_positions, \
            "a still-PENDING, non-AMO exit order must NOT be treated as filled - the position must stay open"
        live = store.live_positions["PAGEIND"]
        assert live.pending_exit_order_id and live.pending_exit_order_id != ote.EXIT_CLAIMED, \
        ...
        print("12. A SELL exit order still PENDING (non-terminal, non-AMO) after the poll budget is "
              ...
              "_sync_pending_orders to resolve later, instead of being silently closed (PAGEIND 17 "
              "Sep 2026 regression): PASSED")
    finally:
        odc.dhan_wrapper.wait_for_order_result = real_wait
        restore()


async def test_13_ltp_stale_for_too_long_forces_a_market_exit_and_cancels_broker_stop():
    """Regression for ANGELONE, 17 Sep 2026 (Swing) / the same class of
    bug as ICICIPRULI, 10 Sep 2026 (Options - see trading-skills'
    incidents/2026-09-10-icicipruli-unmonitorable-position.md, whose own
    "fix direction, not yet built" this implements): Dhan's live-quote
    endpoint can go opaquely dark for a specific contract for many minutes
    ...
    exit-ladder protection since _exit_reason_for never gets a chance to
    run without an LTP - only a resting broker-side stop-loss order still
    protects it independently, if one exists. Once _get_ltp has failed
    CONTINUOUSLY for config.LTP_STALE_FORCE_EXIT_MINUTES, _check_one_
    position now forces a market exit via the same _exit_position pathway
    everything else uses - which also finds and cancels the resting
    broker-side SL-L before placing the fresh SELL, closing both the
    position and its own protective order together, exactly as requested."""
    store = ops.PositionStore()
    ote.position_store = store
    real_enabled = ote.config.BROKER_STOP_LOSS_ENABLED
    ote.config.BROKER_STOP_LOSS_ENABLED = True
    ...
    try:
        ...
        # A single failed tick, not yet stale enough, must NOT force an exit.
        await ote._check_one_position("WIPRO", position)
        assert "WIPRO" in store.live_positions, "one failed LTP tick alone must never force an exit"
        assert cancelled_order_ids == [], "nothing should be touched before the staleness threshold is reached"

        # Fast-forward past the threshold without a real sleep - simulates
        # the failure having been continuous for long enough.
        key = ("WIPRO", position.opened_at)
        ote._ltp_failure_since[key] = datetime.now() - timedelta(
            minutes=ote.config.LTP_STALE_FORCE_EXIT_MINUTES + 1
        )
        await ote._check_one_position("WIPRO", position)

        assert cancelled_order_ids == ["OID-RESTING-SL"], \
            f"the resting broker-side stop-loss must be cancelled as part of the forced exit, got {cancelled_order_ids}"
        assert "WIPRO" not in store.live_positions, "the position must be force-closed once truly stale"
        closed = store.closed_positions_today[0]
        assert closed.exit_reason == "LTP_STALE_FORCED_EXIT", closed.exit_reason
        assert key not in ote._ltp_failure_since, "the staleness tracker must be cleared once the forced exit fires"

        print("13. An LTP fetch failing CONTINUOUSLY for >= LTP_STALE_FORCE_EXIT_MINUTES forces a market "
...
async def main():
    print("=== Options broker-side stop-loss order test suite ===\n")
    ...
    await test_13_ltp_stale_for_too_long_forces_a_market_exit_and_cancels_broker_stop()
    print("\nALL OPTIONS BROKER STOP-LOSS CHECKS PASSED")
