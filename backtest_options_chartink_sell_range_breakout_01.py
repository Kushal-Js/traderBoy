...
buying) side against a real Chartink "Sell" scan alert file ("01 Sell
Range Breakout.csv", 26 Aug - 11 Sep 2026 across 13 trading days, 169
unique symbols). This is the bearish sibling of the CE-only "Range
Breakout" files backtested earlier this session - a "sell" scan alert
routes through Options' /chartink/webhook-sell in production, which buys
PE (not CE) - confirmed via that endpoint's own code
(option_type="PE", prefer_highest=False) before building this script,
since the user initially asked for "CE ATM trades" on this file, which
would have been betting the wrong direction against what the scan
actually signals.
...
CSV_PATH = Path("/root/apps/traderBoy/01 Sell Range Breakout.csv")
CACHE_DIR = Path("/private/tmp/claude-501/-Users-kushalgaur-Desktop-projects-trading-traderBoy/"
                  "60a0e686-2110-4e20-bad5-fe817a57a72a/scratchpad/options_chartink_backtest_cache_sell01")
...
def main():
    ...
    (CACHE_DIR / "results_sell_range_breakout_01.json").write_text(json.dumps(trades, default=str, indent=2))
