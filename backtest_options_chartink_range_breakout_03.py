"""
User request 13 Sep 2026: backtest the OPTIONS strategy against a real
Chartink alert file ("03 Range Breakout.csv", 26 Aug - 11 Sep 2026 across
13 trading days, 103 unique symbols, CE-only scan). Reuses the exact
methodology built for backtest_options_chartink_krishvi_03.py /
...
the Krishvi "03" pass only in which CSV it reads and adds a date-wise
P&L summary alongside the trade-wise list (user explicitly asked for
both this time).
...
session is active):
    uv run python backtest_options_chartink_range_breakout_03.py [PP_BEFORE] [PP_AFTER] [GIVEBACK_PCT]
...
CSV_PATH = Path("/root/apps/traderBoy/03 Range Breakout.csv")
CACHE_DIR = Path("/private/tmp/claude-501/-Users-kushalgaur-Desktop-projects-trading-traderBoy/"
                  "60a0e686-2110-4e20-bad5-fe817a57a72a/scratchpad/options_chartink_backtest_cache_rb03")
...
def main():
    ...
    (CACHE_DIR / "results_range_breakout_03.json").write_text(json.dumps(trades, default=str, indent=2))
