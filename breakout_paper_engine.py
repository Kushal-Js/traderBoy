...
# --------------------------------------------------------------------- #
# The runtime paper-mode ON/OFF switch used to live here (added 24 Sep
# 2026, Options/Futures/Luxury-only at the time, using this module's own
# _hooks() for the .env fallback). Moved out to paper_mode_control.py the
# same day when extended to cover Swing too - a genuinely different
# engine (Swing/swing_paper_engine.py, not this module's STRATEGY
# dispatch table at all) - so the override storage/lookup is now a small
# standalone module neither breakout- nor Swing-specific. Every caller
# (main.py's /paper-mode endpoint, and this module's own STRATEGY.
# *_main.py real-vs-paper dispatch) now imports paper_mode_control
