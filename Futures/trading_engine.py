...
async def reconcile_broker_positions() -> list[Position]:
    ...
    for bp in broker_positions:
        ...
        # incident on Options/JSWENERGY - see Options/trading_engine.py's
        # identical addition and Options/dhan_client.py's get_pending_
        # order_id fix for the full incident writeup): reconciliation
        # otherwise has no way to learn about one, leaving Position.stop_
        # loss_order_id empty and _exit_position's own scan as the only
        # remaining safety net. Best-effort: a failure here just leaves
        # stop_loss_order_id empty, exactly as before this change.
...
async def _process_one_entry(symbol: str, option_type: str) -> dict:
    ...
    if config.ENABLE_RSI_LOSS_REENTRY_BLOCK:
        loss_hits_today = await loop.run_in_executor(
            None, loss_exit_count_today, "Futures", symbol, ("MAX_LOSS_HIT",), datetime.now(),
        )
    ...
    if config.LOSS_REPEAT_BLOCK_ENABLED or config.LOSS_REENTRY_TREND_CHECK_ENABLED:
        loss_count = await loop.run_in_executor(None, loss_count_today, "Futures", symbol, datetime.now())
...
async def _place_broker_stop_loss_if_enabled(
    ...
    """Broker-side SELL stop-loss LIMIT order - see Options/trading_engine.
    py's identical function for the full incident/rationale (18 Sep 2026):
    extracted so both _sync_pending_orders promotion branches below use
    the exact same logic as the fast-fill path, after finding live that
    they were skipping stop-loss placement entirely (not failing it) for
    any BUY order slow enough to miss the poll budget."""
...
async def _enter_single_position(symbol: str, option_type: str = config.OPTION_TYPE) -> dict:
    ...
    if config.LIQUIDITY_ENTRY_GATE_ENABLED:
        ...
        if not passes:
            logger.info(
                "%s: skipped - %s has had a sustained zero-volume streak - not entering an "
                "already-illiquid option",
                symbol, atm.trading_symbol,
            )
            return {
...
async def _get_ltp(trading_symbol: str) -> Optional[float]:
    ...
    Added 18 Sep 2026 (real incident - ABB 29 SEP 7200 CALL, this package):
    if even get_option_ltp fails, falls back once more to the option's own
...
async def _sync_pending_orders() -> None:
    ...
    on live positions, AND (added 15 Sep 2026, real incident on Options/
    ICICIPRULI - see Options/trading_engine.py's identical addition and
    config.STALE_ENTRY_ORDER_TIMEOUT_SECONDS's own docstring) a plain
    market-hours BUY order that's been stuck non-terminal for too long.
    Cheap - normally a no-op, since orders placed during market hours
    almost always resolve within wait_for_order_result's own retry
    budget; this only does real work for a genuinely stuck order."""
    loop = asyncio.get_running_loop()
    ...
    for order in pending_entries:
        ...
        if result.status in OrderStatus.TERMINAL_STATUSES:
            ...
            # _place_broker_stop_loss_if_enabled docstring for the full
            # story: this promotion path used to skip stop-loss placement
            # entirely for any BUY order slow enough to miss the poll budget.
            stop_loss_order_id = await _place_broker_stop_loss_if_enabled(
    ...
    while True:
        try:
            ...
            if config.CLIMACTIC_GUARD_ENABLED:
                await climactic_entry_guard.poll_pending("Futures")
