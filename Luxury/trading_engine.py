...
async def reconcile_broker_positions() -> list[Position]:
    ...
    for bp in broker_positions:
        ...
        # order_id fix for the full incident writeup). Best-effort: a
        # failure here just leaves stop_loss_order_id empty, exactly as
        # before this change.
...
async def _enter_single_position(symbol: str, option_type: str = config.OPTION_TYPE) -> dict:
    ...
    # Options/config.py's own LIQUIDITY_ENTRY_GATE_ENABLED docstring for
    # the full SOLARINDS rationale). Checked as soon as the real contract
    # is known, before the funds check or any real order - the option's
    # OWN liquidity, which neither the volume-floor nor trend-strength
    # gates (both underlying-based) can see.
