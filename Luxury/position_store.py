"""
In-memory, thread/async-safe state for the Luxury strategy's trading day -
entirely independent from Options/position_store.py's and
Futures/position_store.py's own instances (own capacity, own dedup, own
orders). A near-verbatim copy of those files' logic - see their own
docstrings/comments for the full rationale behind each piece; this file
exists separately (rather than importing another package's class with
different config) so each strategy's live-money state can never
...
NOTE: intentionally in-memory, same tradeoff as Options/Futures'
position_store.py.
"""
...
class PositionStore:
    ...
    async def add_position(self, pos: Position) -> None:
        async with self._lock:
            ...
            # attribute this exact broker position back to Luxury (not
            # Options/Futures) during reconciliation.
            fire_and_forget(record_opened_position("Luxury", pos))
            ...
            fire_and_forget(reversal_filters.evaluate_and_log(
                "Luxury", pos.underlying_symbol, pos.option_type, pos.entry_price, pos.order_id,
            ))
            logger.info(
    ...
    async def reconcile_from_broker(self, positions: List[Position]) -> None:
        """Mirrors Options/Futures' identical method - imports positions
        already open at Dhan (already filtered to this strategy's own by
        trading_engine.py's reconcile_broker_positions) into
        live_positions/reserved_symbols."""
        async with self._lock:
    ...
    async def close_position(self, underlying_symbol: str, exit_price: float, reason: str) -> Optional[Position]:
        async with self._lock:
            ...
            if reason == "SUPERTREND_EXIT":
                reversal_filters.record_supertrend_exit("Luxury", pos.underlying_symbol, pos.option_type)
            # Fire-and-forget - see Options/position_store.py's identical
            # comment / trade_history.py's record_closed_trade docstring:
            # must not be awaited while _lock is held.
            fire_and_forget(record_closed_trade("Luxury", pos))
            self.reserved_symbols.pop(underlying_symbol, None)
            logger.info(
